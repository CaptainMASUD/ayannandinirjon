...
// import bandarbanbanner2 from '../../Images/VideoBanner/bandarban2.jpg';
import ayanfbbikebanner from '../../Images/VideoBanner/AyanFbPage.jpg';
import { IoPlayOutline } from 'react-icons/io5'; // Import the play icon
...
function Videos() {
  ...
  // Video data with more videos added
  const videoData = [
  ...
  useEffect(() => {
    setTimeout(() => setLoading(false), 2000); // Set a delay for the loading animation
  }, []);
    ...
    >
      {loading ? (
        ...
        <>
          ...
          {/* Overlay Content */}
          <motion.div
            className="relative z-10 w-full h-full flex flex-col items-center justify-center bg-black bg-opacity-50"
            initial="hidden"
            animate="visible"
            variants={containerVariants}
          >
            ...
            <motion.h1
              className="text-green-500 text-4xl font-bold mb-6"
            ...
            <motion.div
              className="w-11/12 h-[80%] overflow-y-auto mt-6 custom-scrollbar"
              initial="hidden"
              animate="visible"
              variants={containerVariants}
            >
              <motion.div
                ...
                variants={containerVariants}
              >
                {videoData.slice(0, showMore ? videoData.length : window.innerWidth < 640 ? 2 : 3).map((video) => (
                  <motion.div
                    key={video.id}
                    ...
                    {!playingVideo[video.id] ? (
                      <div className="relative w-full h-auto">
                        <img
                          ...
                          style={{ aspectRatio: '16/9' }} // Same aspect ratio as the video player
                        />
